...
// HashInputs computes a hash digest of all data in a SolveOpts that are as function
// inputs to Solve().
//
// The digest returned from this function is the same as the digest that would
// be included with a Solve() Result. As such, it's appropriate for comparison against
// the digest stored in a lock file, generated by a previous Solve(): if the digests match, then manifest
// and lock are in sync, and a Solve() is unnecessary.
//
