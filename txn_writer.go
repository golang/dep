...
func NewSafeWriter(manifest *Manifest, oldLock, newLock *Lock, registryConfig *registryConfig, vendor VendorBehavior) (*SafeWriter, error) {
	sw := &SafeWriter{
		Manifest:       manifest,
		lock:           newLock,
		RegistryConfig: registryConfig,
...
func (sw *SafeWriter) Write(root string, sm gps.SourceManager, examples bool, logger *log.Logger) error {
	...
	if !sw.HasManifest() && !sw.HasRegistryConfig() && !sw.writeLock && !sw.writeVendor {
		// nothing to do
		return nil
	}
	...
	rpath := filepath.Join(root, RegistryConfigName)
	...
	if sw.HasRegistryConfig() {
		// Always write the example text to the bottom of the TOML file.
		tb, err := sw.RegistryConfig.MarshalTOML()
		if err != nil {
			return errors.Wrap(err, "failed to marshal manifest to TOML")
		}
		...
		if err = ioutil.WriteFile(filepath.Join(td, RegistryConfigName), append(initOutput, tb...), 0666); err != nil {
			return errors.Wrap(err, "failed to write manifest file to temp dir")
		}
	}
	...
	if sw.HasRegistryConfig() {
		if _, err := os.Stat(rpath); err == nil {
			// Move out the old one.
			tmploc := filepath.Join(td, RegistryConfigName+".orig")
			failerr = fs.RenameWithFallback(rpath, tmploc)
			if failerr != nil {
				goto fail
			}
			restore = append(restore, pathpair{from: tmploc, to: rpath})
		}

		// Move in the new one.
		failerr = fs.RenameWithFallback(filepath.Join(td, RegistryConfigName), rpath)
		if failerr != nil {
			goto fail
		}
	}
