...
// name beneath root.
//
// - If aManifest is provided, it will be appended to the manifest and validated
// before writing to the standard manifest file.
//
...
func NewSafeWriter(manifest, aManifest *Manifest, oldLock, newLock *Lock, vendor VendorBehavior) (*SafeWriter, error) {
	sw := &SafeWriter{
		Manifest:  manifest,
		aManifest: aManifest,
		lock:      newLock,
...
func (sw *SafeWriter) Write(root string, sm gps.SourceManager, examples bool, logger *log.Logger) error {
	...
	if sw.HasManifest() {
		...
		if sw.aManifest != nil {
			am, err := sw.aManifest.MarshalTOML()
			if err != nil {
				return errors.Wrap(err, "failed to marshal appending manifest to TOML")
			}

			tb = append(tb, am...)
			// validate the final manifest before writing to file.
			if _, err := validateManifest(string(tb)); err != nil {
				return errors.Wrap(err, "failed validating the final manifest after append")
			}
