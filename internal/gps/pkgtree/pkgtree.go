...
// String returns a human-friendly string representing LibraryStatus.
func (ls LibraryStatus) String() string {
	switch ls {
	case NotInTree:
		return "not in tree"
	case NotInLock:
...
// sortedListOfDirectoryChildren returns a lexicographical sorted list of child
// nodes for the specified directory.
func sortedListOfDirectoryChildren(pathname string) (childrenNames []string, err error) {
	...
	// NOTE: Close the file handle to the open directory.
...
func VerifyDepTree(vendorPathname string, expectedSums map[string]string) (status map[string]LibraryStatus, err error) {
	// NOTE: Ensure top level pathname is a directory
	fi, er := os.Stat(vendorPathname)
	if er != nil {
		err = errors.Wrap(er, "cannot Stat")
		return
	}
	if !fi.IsDir() {
		err = errors.Errorf("cannot verify non directory: %q", vendorPathname)
		return
	}

	status = make(map[string]LibraryStatus)
	...
	for len(queue) > 0 {
		...
		short := currentNode.pathname[prefixLength:] // chop off the prefix
		if expectedSum, ok := expectedSums[short]; ok {
			ls := EmptyDigestInLock
			if expectedSum != "" {
				ls = NoMismatch
				actualSum, er := fs.HashFromNode(vendorPathname, short)
				if err != nil {
					err = errors.Wrap(er, "cannot compute dependency hash")
					return
				}
